    ...
    <div className="border-b bg-background">
      <div className="flex min-h-9 items-center gap-2 px-3 py-1.5">
        ...
        >
          ...
          <Badge className="shrink-0" variant={isRunning ? "secondary" : "outline"}>
            {isRunning ? "running" : `${visibleTerminals.length} tabs`}
          </Badge>
          <span className="truncate text-muted-foreground text-xs">
            {activeTerminal?.cwd ?? projectPath ?? projectName ?? "No active project"}
          </span>
        </button>
...
function formatTerminalTitle(terminal: TerminalRecord): string {
  const commandName = terminal.command.split(/[\\/]/).at(-1) ?? terminal.command;
