...
export function buildSkillInvocationPrompt(params: {
  ...
  return [
    ...
    ...sections,
    "User request:",
    params.request.trim() || "Apply the selected skill to the current project state.",
  ]
...
export function buildOutputStylePrompt(params: {
  ...
  return [
    ...
    params.style.description ? `Style description: ${params.style.description}` : "",
...
export function resolveLocalInstructionCommand(params: {
  ...
  if (leadingCommand) {
    ...
    const style = params.outputStyles.find(
      (item) => item.enabled && outputStyleSlashCommandName(item.name) === commandName
    );
    if (style) {
      return {
        command: commandName,
