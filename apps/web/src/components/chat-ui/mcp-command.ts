...
function findTool(
  ...
  return (
    ...
    server.tools.find((tool) => normalizeMatch(tool.name) === normalizeMatch(toolName))
  );
}
...
function clampForPrompt(value: string, maxLength = 12000): string {
...
export function buildMcpToolResultPrompt(params: {
  ...
  return [
    ...
    params.result.truncated ? "Result was truncated by the MCP invocation layer." : "",
    "",
    "Tool arguments:",
