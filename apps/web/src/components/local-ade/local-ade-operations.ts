...
type CheckpointVisualDiffRowLike = {
  kind?: string;
  oldLine?: number;
  newLine?: number;
  oldText?: string;
...
export function getLocalAdeCheckpointRestorePlan(
  ...
  const patchFiles = [
    ...new Set((preview?.diffFiles ?? []).map((file) => file.path).filter(Boolean)),
  ...
  const restorableSafeFiles = sortedSafeFiles.filter((file) => patchFileSet.has(file));
...
export function getLocalAdeCheckpointConflictEditorState(params: {
  ...
  for (const hunk of params.selectedHunks ?? []) {
    selectedHunkCounts.set(hunk.file, (selectedHunkCounts.get(hunk.file) ?? 0) + 1);
  }
  ...
  const rows = files.map((file): LocalAdeCheckpointConflictEditorRow => {
    ...
    const canResolveHunkChoices =
      isTrackedConflict && patch?.hasPatch === true && (patch.hunkCount ?? 0) > 1;
...
export function getLocalAdeCheckpointVisualMergeState(params: {
  ...
  const selectedHunkKeys = new Set(
    (params.selectedHunks ?? []).map(
      (hunk) => `${hunk.file}:${hunk.hunkIndex}`
    )
  );
  ...
  const files = (params.preview?.diffFiles ?? []).map(
    (file): LocalAdeCheckpointVisualMergeFile => {
        ...
        (hunk, hunkIndex): LocalAdeCheckpointVisualMergeHunk => {
          ...
          return {
            ...
            selectable:
              ...
              editorRow?.availableActions.includes("resolve-hunk-choices") === true,
      ...
      const rowCount = hunks.reduce((total, hunk) => total + hunk.rows.length, 0);
  ...
  return {
    ...
    restoreLabel:
      mode === "apply-patch" ? "Restore target" : "Checkpoint side",
    files,
...
function numberResult(
  result: Record<string, string | number | boolean | null | undefined> | undefined,
  key: string
): number | undefined {
    ...
    },
    {
      ...
      action:
        summary.providers.total === 0 || providersReady
          ? "inspect-section"
          : "probe-providers",
      enabled: summary.providers.total === 0 || providersReady || summary.providers.probeTargets.length > 0,
...
function chooseWorkbenchPrimaryAction(
  ...
  if (summary.providers.total === 0 || summary.providers.ready < summary.providers.total) {
...
export function getLocalAdeWorkbenchState(params: {
  ...
  const enabledMemorySources =
    params.snapshot?.projectMemory?.sources?.filter((source) => source.enabled).length ?? 0;
  ...
  return {
    ...
    readyCount,
    totalCount,
    primaryAction: chooseWorkbenchPrimaryAction(actions, summary, params.snapshot),
    metrics: [
      {
        id: "agent",
        label: "Agent",
...
export function getLocalAdeCommandDeckState(params: {
  ...
  const contextMetric = workbench.metrics.find((metric) => metric.id === "context");
...
function toSessionCockpitSession(
  ...
  const pid = typeof session.pid === "number" ? `pid ${session.pid}` : "pid n/a";
  const detailParts = [
    `chat ${shortFocusId(session.id)}`,
    agentSession,
    pid,
  ];
...
export function getLocalAdeSessionCockpitState(params: {
  ...
  const sessions = (params.snapshot?.sessions?.active ?? [])
    ...
    .sort((left, right) => {
      ...
      const rightAttention = right.pendingPermissions * 10 + right.activeToolCalls;
