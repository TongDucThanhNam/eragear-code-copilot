...
export function PromptEnhancementSettingsPanel() {
  ...
  const [form, setForm] =
    useState<PromptEnhancementSettings>(EMPTY_SETTINGS);
  ...
  const updateSettings = trpc.promptEnhancement.updateSettings.useMutation({
    ...
    onError: (error) => {
      toast.error(error.message || "Failed to save prompt enhancement settings");
    },
  });
    ...
    >
      <form className="grid gap-4" onSubmit={handleSubmit}>
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="prompt-enhancement-custom">
            Custom instruction
          </Label>
          <Textarea
            className="min-h-32"
