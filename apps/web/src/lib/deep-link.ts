...
export function parseEragearDeepLink(
  ...
  const projectId =
    queryValue(url, "projectId") ??
    (host === "project" ? segments[0] : null);
...
export async function installEragearDeepLinkHandlers(
  ...
  const pluginModuleName = "@tauri-apps/plugin-deep-link";
  const plugin = await import(/* @vite-ignore */ pluginModuleName).catch(() => null);
  if (!plugin) {
    return () => undefined;
  }
  ...
  const handleUrls = (urls: string[] | null | undefined) => {
    for (const rawUrl of urls ?? []) {
      const action = parseEragearDeepLink(rawUrl);
      if (action) {
        applyEragearDeepLink(action, router);
      }
    }
  };

  await plugin.getCurrent().then(handleUrls).catch(() => undefined);
  const unlisten = await plugin
