...
import type { RuntimeDiagnostics } from "@eragear-code-copilot/shared";
...
interface CheckpointHunkSelection {
  file: string;
  hunkIndex: number;
}
...
interface McpRemoteControlDraft {
...
const HOOK_EVENT_OPTIONS = [
  ...
  {
    value: "after-project-index-refresh",
    label: "after-project-index-refresh",
  },
...
function checkpointHunkSelectionKey(
  selection: CheckpointHunkSelection
): string {
  return `${selection.file}:${selection.hunkIndex}`;
}

function parseMcpHeaderEnvText(
  value: string
  ...
  return (
    <div
      className={cn(
        ...
        workflowLaneToneClass(lane.tone)
      )}
    >
...
function WorkspaceFocusItem({ item }: { item: LocalAdeWorkspaceFocusItem }) {
  return (
    <div
      className={cn(
        "min-w-0 border-l-2 py-1 pl-2",
        workflowLaneToneClass(item.tone)
      )}
    >
  ...
  const handleLaunchCommand = React.useCallback(() => {
    ...
    if (onSubmitCommand) {
      void Promise.resolve(
        ...
        toast.error(
          error instanceof Error ? error.message : "Command launch failed"
        );
      });
      setLaunchArgument("");
      return;
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={statusVariant(cockpit.mode)}>{cockpit.mode}</Badge>
                ...
                <div className="flex min-w-0 items-center justify-between gap-2">
                  ...
                  <Badge
                    variant={statusVariant(
                      workflowLaneBadgeValue(primarySession.tone)
                    )}
                  >
          ...
          {cockpit.sessions.slice(1, 5).map((session) => (
            ...
            >
              <span className="flex min-w-0 items-center justify-between gap-2">
                ...
                <Badge
                  variant={statusVariant(workflowLaneBadgeValue(session.tone))}
                >
                  {session.status}
                </Badge>
              </span>
...
function WorkflowActionStrip({
  ...
  const sessionCockpit = getLocalAdeSessionCockpitState({
    diagnostics,
    snapshot,
  });
      ...
      <div className="border-b bg-muted/20 p-3">
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
          <div className="min-w-0">
            ...
            <div className="mt-1 truncate text-muted-foreground text-xs">
              {commandDeck.detail} /{" "}
              {diagnostics?.endpoint.kind ?? "desktop-service"} /{" "}
          ...
          <div className="flex flex-wrap items-start justify-start gap-1 lg:justify-end">
            ...
            <Badge variant="outline">
              {summary.providers.ready} providers ready
            </Badge>
            <Badge variant="outline">
              {summary.mcp.agentBrokered} brokered MCP
            </Badge>
          </div>
        </div>
      ...
      <div className="grid gap-0 xl:grid-cols-[minmax(420px,0.95fr)_1.35fr]">
        ...
        <div className="p-3">
          <div className="flex flex-wrap items-start justify-between gap-2">
            ...
            <Badge
              variant={statusVariant(snapshot?.capabilities.diagnostics.status)}
            >
...
type BackgroundTask = NonNullable<
  LocalAdeSnapshot["runtime"]["background"]
>["tasks"][number];
    ...
    <div className="rounded-md border bg-background">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b px-3 py-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <Badge
            variant={statusVariant(background.enabled ? "ready" : "disabled")}
          >
          ...
          <span className="text-muted-foreground">
            ...
            {background.startedAt
              ? ` / started ${formatTime(background.startedAt)}`
              : ""}
          </span>
        </div>
      </div>
              ...
              >
                ...
                <div className="min-w-0 text-muted-foreground">
                  ...
                  <div
                    ...
                    title={
                      task.lastError ??
                      backgroundTaskResultText(task.lastResult)
                    }
                  >
                    {task.lastError ??
                      backgroundTaskResultText(task.lastResult)}
...
function RuntimeStrip({
  ...
  const backgroundDetail = dispatch
    ...
    : backgroundSummary.enabled
      ...
      : "not reported";
  const backgroundStatus = backgroundSummary.enabled
  ...
  return (
    <div className="grid gap-2">
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6">
        <StatTile
          detail={
            diagnostics?.health.message ?? "Waiting for runtime diagnostics"
          }
          icon={Activity}
      ...
      {clis.map((cli) => (
        <div className="rounded-md border p-3" key={cli.id}>
          ...
          <div
            ...
            title={cli.executablePath ?? cli.installHint}
          >
            {cli.executablePath
              ? shortPath(cli.executablePath)
              : cli.installHint}
          </div>
        </div>
      ))}
    ...
    <div className="overflow-hidden rounded-md border">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/30 px-3 py-2 text-xs">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          {defaultModel ? (
            <Badge
              variant={statusVariant(snapshot?.runtime.defaultModelStatus)}
            >
              {defaultModel}
            </Badge>
          ) : (
...
function McpManager({
  ...
  const [toolArgsText, setToolArgsText] = React.useState<
  ...
  const runTool = (
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Invalid MCP args JSON"
      );
    }
  };
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">
                  {server.fingerprint}
                  {server.trustedAt
                    ? `; trusted ${formatTime(server.trustedAt)}`
                    : ""}
                </div>
                  ...
                  {remoteControlsAvailable ? (
                    <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_auto_auto]">
                      ...
                      <div className="flex items-end">
                        <Button
                          disabled={
                            !server.enabled || configureRemoteControls.isPending
                          }
                          onClick={() =>
                            configureRemoteControlValues(server, draft)
                          }
                          size="sm"
                          type="button"
                        >
                    ...
                    {server.probeHistory.slice(0, 3).map((run) => (
                      ...
                      >
                        ...
                        <span className="truncate text-muted-foreground">
                          ...
                          {run.toolsDiscovered} tools /{" "}
                          {run.resourcesDiscovered} resources
                      ...
                      .map((run) => (
                        ...
                        >
                          ...
                          <span className="truncate text-muted-foreground">
                            ...
                            {run.notificationCount} notifications; streams{" "}
                            {run.streamOpenCount}; reconnects{" "}
                            {run.reconnectCount}
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="mt-2 grid gap-1 border-t pt-2">
                    ...
                    {server.invocationHistory.slice(0, 3).map((run, index) => (
                      <div
                        ...
                        title={[run.resultText, ...run.diagnostics]
                          .filter(Boolean)
                          .join("\n")}
                      >
                        <Badge variant={statusVariant(run.status)}>
                    ...
                    {server.tools.slice(0, 5).map((tool) => (
                      ...
                      >
                        ...
                        <Textarea
                          ...
                          value={
                            toolArgsText[`${server.id}:${tool.name}`] ?? "{}"
                          }
                        />
                        <Button
                          disabled={
              ...
              <div className="flex shrink-0 items-center gap-2">
                <Button
                  ...
                  type="button"
                  variant={
                    server.trustStatus === "trusted" ? "outline" : "default"
                  }
                >
                  Trust
                </Button>
        ...
        <div className="rounded-md border">
          ...
          <div className="grid gap-2 p-3 text-xs lg:grid-cols-[1fr_1fr]">
            <pre className="max-h-48 overflow-auto rounded bg-muted/40 p-2 font-mono text-[11px]">
              {lastInvocation.resultText ||
                lastInvocation.resultJson ||
                "No content"}
            </pre>
...
function HookRunner({ snapshot }: { snapshot: LocalAdeSnapshot | undefined }) {
  ...
  const [_projectRootAccess, _setProjectRootAccess] = React.useState(true);
  ...
  const reviewHookRun = trpc.settings.reviewHookRun.useMutation({
    onSuccess: (data, variables) => {
      utils.settings.getLocalAdeSnapshot.setData(undefined, data);
      toast.success(
        variables.reviewed ? "Hook run reviewed" : "Hook run reopened"
      );
    },
    onError: (error) => toast.error(error.message),
  });
  ...
  const readyHookBatchItems = hooks
    ...
    .slice(0, 8);
  const hookBatchConfirmed = hookBatchConfirmation.trim() === "RUN HOOK BATCH";
  ...
  const copyHookAudit = async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Hook audit export failed"
      );
    }
  };
    ...
    <div className="space-y-3">
      <div className="rounded-md border bg-muted/20 p-3">
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          <Badge
            variant={
              lifecyclePolicy?.enabled === false ? "secondary" : "default"
            }
          >
            lifecycle{" "}
            {lifecyclePolicy?.enabled === false ? "paused" : "enabled"}
      ...
      <div className="rounded-md border bg-muted/20 p-3">
        <div className="grid gap-3 lg:grid-cols-[minmax(160px,220px)_minmax(160px,220px)_minmax(160px,220px)_1fr]">
          ...
          <div className="flex flex-wrap items-end gap-1">
            <Badge
              variant={
                hookSchedulingPolicy?.enabled === false
                  ? "secondary"
                  : "default"
              }
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      variant={statusVariant(hook.runOperation.approvalStatus)}
                    >
                    ...
                    <Badge
                      variant={
                        hook.envKeys.length > 0 ? "secondary" : "outline"
                      }
                    >
                    ...
                    {hook.lastRun ? (
                      <Badge
                        variant={
                          hook.lastRun.reviewedAt ? "default" : "secondary"
                        }
                      >
                  ...
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                    env{" "}
                    {hook.envKeys.length > 0 ? hook.envKeys.join(", ") : "none"}
                  </div>
                  <div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">
                    {hook.fingerprint}
                    {hook.trustedAt
                      ? `; trusted ${formatTime(hook.trustedAt)}`
                      : ""}
                  </div>
                  <div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">
                    {hook.runOperation.fingerprint}
                  ...
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                    operation {hook.runOperation.event}; cwd{" "}
                    {hook.runOperation.cwd}; approval{" "}
                    {hook.runOperation.approvalStatus}
                  </div>
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                ...
                <div className="grid w-56 shrink-0 gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      disabled={
                        trustHook.isPending || hook.trustStatus === "trusted"
                      }
                      onClick={() =>
                        trustHook.mutate({
                          hookId: hook.id,
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="min-w-0 truncate text-[11px] text-muted-foreground">
                      ...
                      {hook.lastRun.batchId
                        ? ` - batch ${hook.lastRun.batchId}`
                        : ""}
                    </div>
                    <Button
                      disabled={reviewHookRun.isPending}
                      onClick={() => {
                        const lastRun = hook.lastRun;
                        if (!lastRun) {
                          return;
                        }
                        reviewHookRun.mutate({
                          runId: lastRun.id,
                          reviewed: !lastRun.reviewedAt,
                        });
                      }}
                      size="sm"
                      type="button"
            ...
            {visibleHookRuns.slice(0, 8).map((run) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                    {run.event} - {formatTime(run.finishedAt)} -{" "}
                    {run.durationMs}ms
...
function PluginRunner({
  ...
  const [pluginRegistryPackageId, setPluginRegistryPackageId] =
    React.useState("");
  const [runConfirmations, setRunConfirmations] = React.useState<
    Record<string, string>
  >({});
  ...
  const duePluginBatchSchedules = pluginBatchSchedules.filter((schedule) => {
    ...
    return (
      schedule.enabled && Number.isFinite(nextRunMs) && nextRunMs <= Date.now()
    );
  });
  ...
  const runReadyPluginBatch = () => {
    if (pluginBatchCandidates.length === 0) {
      toast.error(
        "No ready trusted plugins are available for batch execution."
      );
      return;
    }
    ...
    <div className="space-y-3">
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(160px,220px)_minmax(160px,220px)_minmax(160px,220px)_1fr]">
          <div className="grid gap-1">
            ...
            <div className="flex h-9 items-center gap-2 rounded-md border bg-background px-3">
              ...
              />
              <span className="text-xs">
                {pluginSchedulingPolicy?.enabled === false
                  ? "paused"
                  : "enabled"}
              </span>
            </div>
      ...
      (pluginDependencyGraph.edges.length > 0 ||
        ...
        <div className="rounded-md border bg-muted/20 p-3">
              ...
              .map((node) => (
                ...
                >
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    {node.dependencyIds.length > 0 ? (
                      <Badge variant="outline">
                        deps {node.dependencyIds.length}
                      </Badge>
                    ) : null}
                    {node.dependentIds.length > 0 ? (
                      <Badge variant="outline">
                        used by {node.dependentIds.length}
                      </Badge>
                    ) : null}
                  </div>
          ...
          {pluginDependencyGraph.diagnostics.length > 0 ? (
            <div className="mt-2 line-clamp-2 text-[11px] text-destructive">
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(220px,300px)_minmax(180px,220px)_auto]">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              <Badge
                variant={
                  pluginBatchCandidates.length > 0 ? "default" : "secondary"
                }
              >
              ...
              <div className="mt-2 grid gap-1">
                {recentPluginBatches.slice(0, 3).map((batch) => (
                  ...
                  >
                    ...
                    <span className="text-muted-foreground">
                      success {batch.counts.success}; failed{" "}
                      {batch.counts.failed}; timeout {batch.counts.timeout};
                      disabled {batch.counts.disabled}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="grid gap-1">
            ...
            <Input
              className="font-mono text-xs"
              onChange={(event) =>
                setPluginBatchConfirmation(event.target.value)
              }
          ...
          <div className="grid gap-1">
            ...
            <Select
              onValueChange={(value) =>
                setPluginBatchFailureMode(
                  value as "continue" | "stop-on-failure"
                )
              }
      ...
      <div className="grid gap-2 rounded-md border bg-muted/20 p-3 lg:grid-cols-[minmax(0,1fr)_auto]">
        <div className="grid gap-1">
          ...
          <div className="truncate text-[11px] text-muted-foreground">
            ...
            schemaVersion, publisher, publisherId, expiry metadata,
            publicKeyPem, signature, and plugin.
          </div>
        </div>
        <div className="flex items-end">
          <Button
                    ...
                    {registry.packages.map((item) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 truncate text-[11px] text-muted-foreground">
                            {item.issuedAt
                              ? `issued ${formatTime(item.issuedAt)}; `
                              : ""}
                            {item.expiresAt
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap gap-1.5 text-[11px] text-muted-foreground">
                    ...
                    {item.envKeys.length > 0 ? (
                      <Badge variant="outline">env {item.envKeys.length}</Badge>
                    ) : null}
                  </div>
                  <div className="mt-1 truncate text-[11px] text-muted-foreground">
                    {item.issuedAt
      ...
      <div className="grid gap-2 xl:grid-cols-[1fr_1fr_1fr_1fr_auto]">
        ...
        <div className="grid gap-1">
          ...
          <div className="flex h-9 items-center gap-2 rounded-md border px-3">
            <Switch
              checked={
                policyPreset === "restricted" ? false : projectRootAccess
              }
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      variant={statusVariant(plugin.executionPolicy.status)}
                    >
                    ...
                    {plugin.packageGovernanceStatus ? (
                      <Badge
                        variant={statusVariant(plugin.packageGovernanceStatus)}
                      >
                    ...
                    {plugin.dependencyIds.length > 0 ? (
                      <Badge variant="outline">
                        deps {plugin.dependencyIds.length}
                      </Badge>
                    ) : null}
                    <Badge
                      variant={
                        plugin.envKeys.length > 0 ? "secondary" : "outline"
                      }
                    >
                    ...
                    {plugin.lastRun ? (
                      <Badge
                        variant={
                          plugin.lastRun.reviewedAt ? "default" : "secondary"
                        }
                      >
                  ...
                  <div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">
                    {plugin.fingerprint}
                    {plugin.trustedAt
                      ? `; trusted ${formatTime(plugin.trustedAt)}`
                      : ""}
                  </div>
                  <div className="mt-1 truncate font-mono text-[11px] text-muted-foreground">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {plugin.installSource === "signed-package" ? (
                      <Button
                        ...
                        onClick={() =>
                          revalidatePluginPackage.mutate({
                            pluginId: plugin.id,
                          })
                        }
                        size="sm"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        runPlugin.mutate({
                          ...
                          confirmation: runConfirmation,
                          operationApprovalId:
                            plugin.runOperation.approvalId ?? "",
                        })
                      }
                      size="sm"
                      type="button"
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="min-w-0 truncate text-[11px] text-muted-foreground">
                      ...
                      {plugin.lastRun.batchId
                        ? `; batch ${plugin.lastRun.batchId}`
                        : ""}
                    </div>
                    <Button
                      disabled={reviewPluginRun.isPending}
                      onClick={() => {
                        const lastRun = plugin.lastRun;
                        if (!lastRun) {
                          return;
                        }
                      ...
                      <div className="mb-1 flex flex-wrap items-center gap-2">
                        ...
                        {plugin.lastRun.postRunCheckpointId ? (
                          <Badge variant="secondary">
                            post{" "}
                            {plugin.lastRun.postRunCheckpointId.slice(0, 18)}
                          </Badge>
                        ) : null}
                      </div>
                      <div className="text-[11px] text-muted-foreground">
                        before{" "}
                        {plugin.lastRun.workspaceStatusBefore?.length ?? 0}
                        {" -> "}after{" "}
                        {plugin.lastRun.workspaceStatusAfter?.length ?? 0}
                      </div>
                      {(plugin.lastRun.workspaceChangedFiles?.length ?? 0) >
                      0 ? (
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="truncate text-muted-foreground text-xs">
            {shortPath(index?.storagePath)} -{" "}
            {index?.indexedAt
              ? `indexed ${formatTime(index.indexedAt)}`
              : "not indexed"}
          </div>
        </div>
        <Button
      ...
      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6">
        ...
        <StatTile
          detail={
            tasks
              .slice(0, 2)
              .map((item) => `${item.marker} ${item.path}`)
              .join(", ") || "No tasks"
          }
          icon={ShieldAlert}
          label="Tasks"
...
function MemoryAndTrust({
  ...
  const [memoryPresetMaxBytes, setMemoryPresetMaxBytes] =
    React.useState("12000");
  ...
  const upsertProjectMemoryPreset = trpc.memory.upsertPreset.useMutation({
    onSuccess: (data) => {
      updateProjectMemoryCache(data);
      setMemoryPresetName("");
  ...
  const deleteProjectMemoryPreset = trpc.memory.deletePreset.useMutation({
    onSuccess: (data) => {
      updateProjectMemoryCache(data);
  ...
  const restoreCheckpointFiles =
    trpc.settings.restoreCheckpointFiles.useMutation({
      onSuccess: (data) => {
        utils.settings.getLocalAdeSnapshot.setData(undefined, data);
        setCheckpointPreview(null);
        setRestoreConfirmation("");
  ...
  const restoreCheckpointHunks =
    trpc.settings.restoreCheckpointHunks.useMutation({
      onSuccess: (data) => {
        utils.settings.getLocalAdeSnapshot.setData(undefined, data);
        setCheckpointPreview(null);
        setRestoreConfirmation("");
        ...
        <div className="rounded-md border p-3">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {memorySources.map((source) => (
              ...
              >
                <Checkbox
                  checked={memoryPresetSourcePaths.includes(
                    source.relativePath
                  )}
                  onCheckedChange={(checked) =>
                    toggleMemoryPresetSource(
                      source.relativePath,
                      checked === true
                    )
                  }
              ...
              {memoryPresets.map((preset) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-muted-foreground">
                      {preset.sourcePaths.join(", ")} -{" "}
                      {formatBytes(preset.maxBytes)}
      ...
      <div className="space-y-2">
        <div className="rounded-md border p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge
              variant={snapshot?.changeTrust.isGitRepo ? "default" : "outline"}
            >
          ...
          <Button
            ...
            disabled={
              ...
              snapshot?.changeTrust.isGitRepo === false
            }
          ...
          <div className="mt-2 max-h-48 overflow-y-auto rounded bg-muted/30 p-2 font-mono text-[11px]">
            {(snapshot?.changeTrust.statusLines ?? [])
              .slice(0, 24)
              .map((line) => (
        ...
        <div className="rounded-md border p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <Badge variant="outline">
              {snapshot?.checkpoints.items.length ?? 0}
            </Badge>
          </div>
          <div className="mt-2 max-h-48 space-y-2 overflow-y-auto">
            {(snapshot?.checkpoints.items ?? [])
              .slice(0, 8)
              .map((checkpoint) => (
                <div className="rounded border p-2" key={checkpoint.id}>
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <Badge
                        variant={checkpoint.canRestore ? "default" : "outline"}
                      >
                    ...
                    .map((restore) => (
                      ...
                      >
                        ...
                        {restore.safetyCheckpointId
                          ? ` - safety ${restore.safetyCheckpointId}`
                          : ""}
                      </div>
                    ))}
          ...
          {checkpointPreview ? (
            <div className="mt-2 rounded-md border bg-muted/20 p-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <Badge
                  variant={
                    checkpointPreview.canRestore ? "default" : "secondary"
                  }
                >
                  {checkpointPreview.canRestore
                    ? "restore ready"
                    : "restore blocked"}
                </Badge>
              </div>
              <div className="mt-2 rounded border bg-background/60 p-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      disabled={
                        !checkpointRestorePlan.canRestoreSelectedSafeFiles
                      }
                      onClick={() => {
                        setSelectedCheckpointFiles(
                ...
                {checkpointRestorePlan.trackedConflictFiles.length > 0 ? (
                  <div className="mt-1 truncate text-[11px] text-blue-600 dark:text-blue-300">
                    tracked conflicts:{" "}
                    <span className="font-mono">
                  ...
                  <div className="max-h-72 divide-y overflow-y-auto">
                    {checkpointConflictEditor.rows.map((row) => {
                      ...
                      const canResolveHunkChoices =
                        row.availableActions.includes("resolve-hunk-choices");
                      const canToggleHunks =
                        canRestoreHunks || canResolveHunkChoices;
                ...
                <div className="mt-2 overflow-hidden rounded border bg-background/70">
                  ...
                  <div className="max-h-[32rem] divide-y overflow-y-auto">
                    {checkpointVisualMerge.files.map((file) => {
                      ...
                      const canSelectHunks = file.hunks.some(
                        (hunk) => hunk.selectable
                      );
                      const allHunksSelected =
                        file.hunkCount > 0 &&
                        file.selectedHunks === file.hunkCount;
                      return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2 border-b bg-background px-2 py-1.5">
                        <div className="flex min-w-0 items-start gap-2">
                          <Checkbox
                            ...
                            className="mt-0.5"
                            disabled={
                              checkpointRiskByFile.get(file.path)?.level !==
                              "safe"
                            }
                            onCheckedChange={(checked) =>
                              toggleCheckpointFileSelection(
                      ...
                      <div className="max-h-72 overflow-auto">
                        {file.hunks.map((hunk, hunkIndex) => {
                          ...
                          return (
                            <div
                              key={`${file.path}:${hunk.header}:${hunkIndex}`}
                            >
                              <div className="flex items-center gap-2 border-b bg-muted/30 px-2 py-1 text-[11px] text-muted-foreground">
                                <Checkbox
                                  ...
                                  checked={selectedCheckpointHunkSet.has(
                                    hunkSelectionKey
                                  )}
                                  disabled={
                                    !isCheckpointHunkSelectable(file.path)
                                  }
                                  onCheckedChange={(checked) =>
                                    toggleCheckpointHunkSelection(
                                      hunkSelection,
              ...
              {(checkpointPreview.sessionAttributions ?? []).length > 0 ? (
                <div className="mt-2 overflow-hidden rounded border">
                  ...
                  <div className="max-h-32 overflow-y-auto">
                    {(checkpointPreview.sessionAttributions ?? []).map(
                      (session) => (
                        <div
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          {risk.checkpointStatus || risk.currentStatus ? (
                            <div className="truncate text-muted-foreground">
                              checkpoint {risk.checkpointStatus ?? "clean"} /
                              current {risk.currentStatus ?? "clean"}
                            </div>
                          ) : null}
                        </div>
                        <span className="truncate text-muted-foreground">
                  ...
                  <Button
                    disabled={
                      ...
                      restoreConfirmation.trim() !==
                        checkpointPreview.restoreToken
                    }
                    onClick={() =>
                      shelveCheckpointConflicts.mutate({
...
function AcpActivityPanel({
  ...
  const retryAcpActivityStream =
    trpc.settings.retryAcpActivityStream.useMutation({
      onSuccess: (data) => {
        utils.settings.getLocalAdeSnapshot.setData(undefined, data);
      },
    });
  ...
  const handleCopyTrace = React.useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "ACP trace export failed"
      );
    }
  ...
  const handleReplayTrace = React.useCallback(
    async (
      ...
      try {
        ...
        toast.error(
          error instanceof Error ? error.message : "ACP replay failed"
        );
      }
    },
    ...
    try {
      ...
      await saveAcpReplayPreset.mutateAsync({
        ...
        ...(filters.correlationKey
          ? { correlationKey: filters.correlationKey }
          : {}),
  ...
  return (
    <div className="rounded-md border">
          ...
          <div className="grid gap-1.5 md:grid-cols-4">
            <div className="rounded border px-2 py-1.5 text-xs">
              ...
              <div className="font-medium">
                {stream.latestTimestamp
                  ? formatTime(stream.latestTimestamp)
                  : "n/a"}
              </div>
              <div className="text-[11px] text-muted-foreground">
            ...
            <div className="rounded border px-2 py-1.5 text-xs">
              <div className="text-[11px] text-muted-foreground">Silence</div>
            ...
            <div className="rounded border px-2 py-1.5 text-xs">
              <div className="text-[11px] text-muted-foreground">Causality</div>
              <div className="font-medium">
                {stream.rootCount} roots / {stream.longestChainLength} longest
              </div>
              <div className="text-[11px] text-muted-foreground">
                {stream.correlatedFrameCount} correlated /{" "}
                {stream.orphanFrameCount} orphan
              </div>
            </div>
            <div className="rounded border px-2 py-1.5 text-xs">
              <div className="text-[11px] text-muted-foreground">Heartbeat</div>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="flex items-center justify-end gap-1">
                      <Badge
                        variant={warningCount > 0 ? "secondary" : "outline"}
                      >
                        {chain.eventCount}
                      </Badge>
                      <Button
            ...
            <div className="mt-2 rounded border bg-background px-2 py-1.5 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span
                  ...
                  title={currentReplayFrame.message}
                >
      ...
      {visibleReplayKinds.length > 0 ? (
        <div className="border-b p-2">
          <div className="mb-1.5 font-medium text-[11px] text-muted-foreground uppercase">
            Replay by kind
          </div>
          <div className="flex flex-wrap gap-1.5">
            {visibleReplayKinds.map(([kind, count]) => (
              <Button
                ...
                type="button"
                variant={
                  replay?.filters.kind === kind ? "secondary" : "outline"
                }
              >
                <Play className="mr-1 h-3 w-3" />
                {kind} {count}
      ...
      <div className="rounded-md border">
        ...
        <div className="max-h-64 overflow-y-auto p-2">
          {(snapshot?.logs.entries ?? []).map((entry) => (
            <div
              className="grid grid-cols-[56px_54px_1fr] gap-2 rounded px-2 py-1 text-xs"
              key={entry.id}
            >
              <span className="text-muted-foreground">
                {formatTime(entry.timestamp)}
...
function LocalAdeSectionsSurface({
  ...
  const [diagnostics, setDiagnostics] =
    React.useState<RuntimeDiagnostics | null>(
  ...
  const refreshDiagnostics = React.useCallback(async () => {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Diagnostics refresh failed"
      );
    }
  }, [refetchSnapshot]);
  ...
  const operationSummary = getLocalAdeOperationSummary({
    diagnostics,
    snapshot,
  });
  ...
  const handleProbeMcpServers = React.useCallback(async () => {
    const servers = (snapshot?.mcp.servers ?? []).filter(
  ...
  const hasSection = (section: LocalAdeSectionId) =>
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 truncate text-muted-foreground text-sm">
              {shortPath(snapshot?.projectRoot)} - Electron IPC/private
              desktop-service
            </p>
          </div>
          <div className="flex items-center gap-2">
      ...
      {hasSection("providers") ? (
        ...
        >
          <Section icon={Terminal} title="Agent CLI Detection">
            <CliGrid diagnostics={diagnostics} />
          </Section>

          <Section
            icon={KeyRound}
            id="local-ade-providers"
            title="Provider And Agent State"
          >
      ...
      {hasSection("hooks") ? (
        <Section
          action={
            <Badge variant="outline">{snapshot?.hooks.items.length ?? 0}</Badge>
          }
          icon={Play}
          title="Hooks"
        >
      ...
      {hasSection("plugins") ? (
        <Section
          action={
            <Badge variant="outline">
              {snapshot?.plugins.items.length ?? 0}
            </Badge>
          }
          icon={PlugZap}
          title="Plugins"
      ...
      {hasMemoryOrMcp ? (
        ...
        >
          {hasSection("memory") ? (
            <Section
              icon={FileText}
              id="local-ade-change-trust"
              title="Project Memory And Change Trust"
            >
          ...
          {hasSection("mcp") ? (
            <Section icon={PlugZap} id="local-ade-mcp" title="MCP Servers">
              <McpManager
                onProbe={() => {
                  void refreshDiagnostics();
      ...
      {hasSection("project-index") ? (
        <Section
          action={
            <Badge
              variant={snapshot?.projectIndex.indexedAt ? "default" : "outline"}
            >
      ...
      {hasSection("activity") ? (
        <Section icon={GitBranch} title="Runtime Logs And Dashboard Parity">
      ...
      {hasSection("storage") ? (
        <div className="grid gap-2 sm:grid-cols-3">
          ...
          <StatTile
            detail={
              snapshot?.blockers.map((item) => item.workflow).join(", ") ||
              "No local blockers"
            }
            icon={ShieldAlert}
...
export function LocalAdeAutomationSettingsPanel() {
  return (
    <LocalAdeSettingsPanel sections={LOCAL_ADE_AUTOMATION_SETTINGS_SECTIONS} />
  );
}

export function LocalAdeMemorySettingsPanel() {
  return (
    <LocalAdeSettingsPanel sections={LOCAL_ADE_MEMORY_SETTINGS_SECTIONS} />
  );
}
...
export function LocalAdeActivitySettingsPanel() {
  return (
    <LocalAdeSettingsPanel sections={LOCAL_ADE_ACTIVITY_SETTINGS_SECTIONS} />
  );
}
