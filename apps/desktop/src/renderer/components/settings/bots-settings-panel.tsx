...
import { Bot, Play, RefreshCw, RotateCcw, Square, Trash2 } from "lucide-react";
        ...
        >
          ...
          {form.trigger === "quota_refresh" ? (
            <div className="grid gap-3 rounded-md border bg-background p-3">
              <div className="font-medium text-xs uppercase">Quota Trigger</div>
              <div className="grid gap-3 sm:grid-cols-2">
  ...
  return (
    <div className="grid gap-3 rounded-md border bg-background p-3">
      ...
      <div className="flex flex-wrap gap-2 text-xs">
        ...
        <Badge variant="outline">
          {formatExecutionTarget(bot.execution.target)}
        </Badge>
        {bot.agentId ? (
          <Badge variant="outline">agent {bot.agentId}</Badge>
        ) : null}
        {bot.projectId ? (
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border bg-background p-3">
      <div className="min-w-0">
        ...
        <div className="mt-1 truncate text-muted-foreground text-xs">
          ...
          {run.nextAttemptAt
            ? ` · retry ${formatTimestamp(run.nextAttemptAt)}`
            : ""}
        </div>
        <div className="mt-1 flex flex-wrap gap-2 text-muted-foreground text-xs">
          ...
          {run.triggerContext?.resetAt ? (
            <span>
              reset {formatTimestamp(Date.parse(run.triggerContext.resetAt))}
            </span>
          ) : null}
        </div>
        {run.error ? (
...
function formatQuotaConfig(
  ...
  const windows =
    quota.windowIds.length > 0 ? quota.windowIds.join("/") : "any";
