...
import { useChatMessageCount } from "@/store/chat-stream-store";
    ...
    <div className="flex flex-wrap items-center gap-2 border-b px-3 py-2 text-xs">
      {recent.map((invocation) => (
        ...
        >
          <Badge
            variant={
              invocation.status === "failed" ? "destructive" : "secondary"
            }
          >
            {invocation.status}
          </Badge>
...
export function ChatInterface({
  ...
  const { data: slashCommandRegistry } = trpc.commands.list.useQuery(
    undefined,
    {
      staleTime: 30_000,
    }
  ...
  const availableModels = useMemo(() => {
    const mapped = (selectionState.models?.availableModels ?? []).map(
      (model) => ({
    ...
    return [
      {
        ...
        description:
          "Invoke a trusted MCP tool and send the result to the agent",
        input: {
    ...
    () => [
      {
        ...
        description:
          "Search Project Index and send matched symbols/tasks to the agent",
    ...
    async (message: PromptInputMessage) => {
      ...
      if (projectMemoryCommand) {
        ...
        if (memoryContext.status === "no-enabled-sources") {
          toast.error(
            "Enable a project memory source in Settings > Memory first."
          );
      ...
      const projectIndexCommand =
        projectMemoryPrompt || mcpCommandPrompt
          ? null
      ...
      if (projectIndexCommand) {
        ...
        if (indexSearch.status === "no-results") {
          toast.info(
            "No direct Project Index matches; sending the no-match context."
          );
        }
      ...
      const subagentCommand =
        projectMemoryPrompt || projectIndexPrompt || mcpCommandPrompt
          ? null
          : resolveSubagentCommand({
              text: message.text,
