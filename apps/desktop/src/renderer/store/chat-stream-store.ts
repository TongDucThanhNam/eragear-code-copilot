...
const EMPTY_TERMINAL_SELECTION: readonly (
  | string
...
export function useChatMessages(
  ...
  return useChatStreamStore((state) => {
    ...
    return (
      state.byChatId[chatId]?.messageState.orderedMessages ?? EMPTY_MESSAGES
    );
  });
}
...
export function useChatMessageById(
  ...
  return useChatStreamStore((state) => {
    ...
    return (
      state.byChatId[chatId]?.messageState.byId.get(messageId) ?? EMPTY_MESSAGE
    );
  });
}
