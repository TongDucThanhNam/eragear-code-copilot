...
function reconcileLateTerminalMessageSnapshot(
  ...
  return (
    finalizeStreamingMessagesInState(replaceMessagesState([message])).byId.get(
      message.id
    ) ?? message
  );
}
  ...
  return useCallback(
    (event: BroadcastEvent) => {
      if (
        ...
        const meta: Record<string, unknown> = isModeEvent
          ? {
              eventType: event.type,
      ...
      const _diagProcessStart = isClientDiagnosticsEnabled()
        ? performance.now()
        : 0;
      processSessionEvent(
        event,
              ...
              if (nextState === prev) {
                ...
                if (isTextualAppliedUpdate) {
                  const incomingTextLength =
                    "text" in partEvent.part
                      ? partEvent.part.text.length
                      : null;
                  const meta = {
                    chatId: activeChatIdRef.current,
                    messageId: partEvent.messageId,
      ...
      if (event.type === "chat_finish") {
        ...
        if (reloadDecision.shouldReload) {
          ...
          if (import.meta.env.DEV) {
            console.warn(
              "[ACP Session Event] loadHistory(force=true) after chat_finish",
              {
                chatId: activeChatIdRef.current,
                turnId: event.turnId ?? null,
                messageId: reloadDecision.resolvedMessageId,
