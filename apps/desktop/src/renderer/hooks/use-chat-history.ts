...
export function normalizeOlderHistoryBatchOrder(
  ...
  const lastKnown = knownIndexes.at(-1);
  if (
  ...
  const loadHistory = useCallback(
    async (force = false) => {
      ...
      if (!activeChatId || readOnly || isResumingRef.current) {
        chatDebug(
          "history",
          "loadHistory skipped: inactive/readOnly/resuming",
          {
            chatId: activeChatId,
            readOnly,
      ...
      if (!force && connStatus !== "connecting" && connStatus !== "connected") {
        chatDebug(
          "history",
          "loadHistory skipped: connection status not ready",
          {
            chatId: activeChatId,
            connStatus,
            force,
      ...
      chatDebug(
        "history",
        "loadHistory started (source=db:getSessionMessagesPage)",
        {
          chatId: activeChatId,
          force,
        }
      ...
      return runSharedInFlightLoad(historyInFlightRef, async () => {
        try {
          ...
          chatDebug("history", "loadHistory page fetched from db", {
            chatId: activeChatId,
            force,
            messageCount: Array.isArray(page.messages)
          ...
          chatDebug("history", "loadHistory messages normalized", {
            ...
            lastMessageId: normalizedMessages.at(-1)?.id ?? null,
          });
          if (normalizedMessages.length > 0) {
            ...
            if (
              ...
              chatDebug(
                "permission",
                "history snapshot merged into client state",
                {
                  chatId: activeChatId,
                  force,
                  normalizedCount: normalizedMessages.length,
                  ...
                  nextPendingRequestId:
                    nextPendingPermission?.requestId ?? null,
                  mergeMode:
