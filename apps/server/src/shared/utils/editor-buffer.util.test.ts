...
describe("pruneEditorTextBuffers", () => {
  test("evicts stale buffers by ttl", () => {
    ENV.editorBufferTtlMs = 1_000;
    ENV.editorBufferMaxFilesPerSession = 10;

    const session = {
      editorTextBuffers: new Map([
        ["/tmp/old.ts", { content: "old", updatedAt: 1_000 }],
        ["/tmp/fresh.ts", { content: "fresh", updatedAt: 4_500 }],
      ]),
    };

    pruneEditorTextBuffers(session, 5_000);
  ...
  test("evicts oldest buffers when exceeding max files", () => {
    ...
    const session = {
      editorTextBuffers: new Map([
        ["/tmp/1.ts", { content: "1", updatedAt: 1_000 }],
        ["/tmp/2.ts", { content: "2", updatedAt: 2_000 }],
        ["/tmp/3.ts", { content: "3", updatedAt: 3_000 }],
      ]),
    };

    pruneEditorTextBuffers(session, 3_000);
