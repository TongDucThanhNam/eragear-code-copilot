...
type MessageRow = typeof sqliteSchema.sessionMessages.$inferSelect;
type SessionInsert = typeof sqliteSchema.sessions.$inferInsert;
type MessageInsert = typeof sqliteSchema.sessionMessages.$inferInsert;
...
export class SessionSqliteRepository implements SessionRepositoryPort {
  ...
  private mapSessionListRow(row: SessionListRow): StoredSession {
    return {
      id: row.id,
  ...
  private mapSessionRow(row: SessionRow): StoredSession {
    return {
      id: row.id,
      name: row.name ?? undefined,
      sessionId: row.sessionId ?? undefined,
  ...
  private toSessionInsert(session: StoredSession): SessionInsert {
    return {
      id: session.id,
