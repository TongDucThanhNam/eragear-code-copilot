...
async function makeStore() {
  ...
  const store = new EncryptedCredentialFileStore({
  ...
  return {
    filePath: path.join(tempDir, "credentials.json"),
    store,
    service: new CredentialService(store, {
      createId: () => "cred-1",
      nowMs: () => 100,
    }),
  };
...
describe("EncryptedCredentialFileStore", () => {
  test("stores encrypted secrets and returns only redacted records", async () => {
    const { filePath, service } = await makeStore();
  ...
  test("deletes only credentials owned by the user", async () => {
    ...
    const deleted = await service.delete("user-1", { id: record.id });
    const listed = await service.list("user-1");
