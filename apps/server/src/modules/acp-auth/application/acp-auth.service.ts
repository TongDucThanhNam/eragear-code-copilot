...
export class AcpAuthService {
  ...
  async list(
    ...
    const providers = await this.repository.list(userId, input);
  ...
  async delete(userId: string, input: { providerId: string }): Promise<{
    deleted: true;
  }> {
    const existing = await this.repository.get(userId, input.providerId);
    if (existing) {
      await this.repository.removeProviderAuthFile(existing);
    }
    await this.repository.delete(userId, input);
    return { deleted: true };
  }
  ...
  async syncStartup(): Promise<AcpAuthSyncResult> {
    const providers = await this.repository.listAll({ includeDisabled: true });
    return await this.syncRecords(providers);
  }
  ...
  private async syncRecord(
    ...
    if (!record.enabled) {
      ...
      return await this.repository.updateSyncState(
        record.userId,
        record.providerId,
        {
          syncStatus: "disabled",
