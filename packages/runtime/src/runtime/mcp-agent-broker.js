// biome-ignore-all lint/performance/useTopLevelRegex: Broker protocol parsing keeps local regex literals near the redaction/parsing logic for readability.
// biome-ignore-all lint/complexity/noExcessiveCognitiveComplexity: Broker request handlers are preserved during runtime extraction to avoid ACP/MCP behavior changes.
// biome-ignore-all lint/complexity/noVoid: Fire-and-forget broker shutdown handling is intentional.
...
function _parseJsonRpcError(error) {
  if (!isRecord(error)) {
...
function redactJsonValue(value, secrets) {
  ...
  try {
    ...
    return {
      jsonrpc: "2.0",
      error: { code: -32_000, message: serialized },
    };
  }
}
...
async function fetchWithTimeout(
  url,
  options,
  timeoutMs = MCP_PROTOCOL_TIMEOUT_MS
) {
...
async function main() {
  ...
  if (!(projectRoot && serverId && expectedFingerprint)) {
    throw new Error(
      "MCP broker requires project-root, server-id, and fingerprint."
    );
  }
  ...
  async function postSseMessage(currentServer, message, secretsForMessage) {
    ...
    if (!response.ok) {
      throw new Error(
        `HTTP ${response.status}: ${redact(text, secretsForMessage)}`
      );
    }
    if (text.trim()) {
      settleSseMessage(
  ...
  async function auditRemoteResult(params) {
    await appendAudit(projectRoot, {
      ...
      ...(params.responseMessage
        ? {
            resultText: redact(
              resultText(params.responseMessage, secrets),
              secrets
            ),
          }
        : {}),
