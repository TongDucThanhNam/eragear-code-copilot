...
import type { LogLevel } from "#runtime/shared/types/log.types";
import { isAcpLogMessage } from "#runtime/shared/utils/acp-log.util";
import { createId } from "#runtime/shared/utils/id.util";
import type { TRPCContext } from "#runtime/transport/trpc/context";
import { appRouter } from "#runtime/transport/trpc/router";
import { createRuntimeCoreFromSettings, type RuntimeCore } from "./core";
...
function levelFromConsoleMethod(
...
function appendDesktopServiceLog(
  ...
  const chatId =
    typeof context.chatId === "string" ? context.chatId : undefined;
  const userId =
...
async function ensureLocalDefaults(core: RuntimeCore): Promise<void> {
  ensureLocalDefaultsPromise ??=
    core.composition.deps.useCases.agent.ensureDefaults.execute(
      LOCAL_DESKTOP_USER_ID
    );
  await ensureLocalDefaultsPromise;
}

async function createDesktopTrpcContext(
  core: RuntimeCore
): Promise<TRPCContext> {
...
async function handleRequest(
  ...
  switch (message.kind) {
    ...
    default:
      throw new Error("Unsupported desktop service message kind.");
  }
}
