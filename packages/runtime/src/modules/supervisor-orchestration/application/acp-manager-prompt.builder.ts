...
export function buildAcpManagerPrompt(input: {
  ...
  return [
    ...
    "Each task must have taskId, title, goal, role, executionMode, dependencies, criterionIds, changeKinds, optional candidateAgentId, optional preferredModelId, scopeIntent, and verificationRequirements.",
    ...
    "task.dependencies, task.criterionIds, task.changeKinds, task.scopeIntent, and task.verificationRequirements must each be JSON arrays of strings, including when there is only one item; risks must also be an array of strings.",
    "When context.goalContract is present, every acceptance criterion id must appear in at least one task.criterionIds, and no other criterion id is allowed. Put machine-evidence criteria on a task that has runtime-trusted verification; user-evidence criteria still require explicit user acceptance after their covering tasks finish.",
    "Allowed task.changeKinds are scoped_code_change, architecture_change, dependency_change, and final_integration. Every write task must declare scoped_code_change; read-only tasks must use changeKinds=[]. Declare architecture_change or dependency_change whenever the task proposes one. When goalContract.authority.finalIntegration=ask, at least one write task must declare final_integration so exact Plan approval is the durable authority boundary.",
    "A Goal Contract authority value of ask is not permission. The plan must visibly declare that change kind and wait for exact Plan approval; do not hide an architecture, dependency, scoped-code, or final-integration change in prose.",
