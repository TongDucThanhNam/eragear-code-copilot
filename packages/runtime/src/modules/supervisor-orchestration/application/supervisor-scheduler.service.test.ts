...
describe("SupervisorSchedulerService", () => {
  test("dispatches independent tasks in parallel up to the persisted cap", () => {
    const decision = new SupervisorSchedulerService().evaluate(
      createRun(),
  ...
  test("dispatches nothing while paused or after the run deadline", () => {
    const scheduler = new SupervisorSchedulerService();
    expect(
      scheduler.evaluate(
        createRun({ status: "paused" }),
        Date.parse("2026-07-11T00:01:00.000Z")
      ).dispatchTaskIds
    ).toEqual([]);
    const expired = scheduler.evaluate(
      createRun(),
      Date.parse("2026-07-12T01:00:00.000Z")
    );
    expect(expired.deadlineExceeded).toBeTrue();
    expect(expired.dispatchTaskIds).toEqual([]);
  });
  ...
  test("does not retry a task after its attempt budget is exhausted", () => {
    ...
    expect(
      new SupervisorSchedulerService().evaluate(
        run,
        Date.parse("2026-07-11T00:01:00.000Z")
      ).readyTaskIds
