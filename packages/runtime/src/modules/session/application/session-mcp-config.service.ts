...
export function resolveMcpAgentBrokerRuntimeCommand(
  ...
  return isJavaScriptRuntimeExecutable(process.execPath)
    ? process.execPath
    : "bun";
}

export function resolveMcpAgentBrokerScript(
  options: { cwd?: string; env?: NodeJS.ProcessEnv; moduleDir?: string } = {}
): string {
...
function visibleProjectLocalMcpRemoteControls(
  ...
  const controls = normalizeProjectLocalMcpRemoteControls(
    server.remoteControls
  );
  return {
...
function parseProjectLocalMcpServer(
  value: unknown
): ProjectLocalMcpServer | null {
  if (
    !isRecord(value) ||
  ...
  return {
    ...
    ...(sanitizeRecord(value.headers)
      ? { headers: sanitizeRecord(value.headers) }
      : {}),
    ...
    ...(normalizeProjectLocalMcpRemoteControls(value.remoteControls)
      ? {
          remoteControls: normalizeProjectLocalMcpRemoteControls(
            value.remoteControls
          ),
        }
      : {}),
...
function missingHeaderEnvKeys(server: ProjectLocalMcpServer): string[] {
  return Object.values(server.headerEnv ?? {}).filter(
...
export class SessionMcpConfigService {
  ...
  async resolveServers(
    ...
    const projectLocalMcpServers = (
      await readProjectLocalMcpServers(projectRoot)
    )
