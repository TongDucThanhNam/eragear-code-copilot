...
import type { StoredSession } from "#runtime/modules/session/domain/stored-session.types";
import type { getSqliteOrm } from "#runtime/platform/storage/sqlite-db";
import { sqliteSchema } from "#runtime/platform/storage/sqlite-db";
...
function createSessionListSelect(db: SqliteOrm) {
  return db
    .select({
      ...
      envMode: sqliteSchema.sessions.envMode,
      worktreePath: sqliteSchema.sessions.worktreePath,
      worktreeBranch: sqliteSchema.sessions.worktreeBranch,
