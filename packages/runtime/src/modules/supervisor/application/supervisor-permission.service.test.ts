// biome-ignore-all lint/suspicious/useAwait: Async test doubles preserve the supervisor service port shapes.
import { describe, expect, test, vi } from "bun:test";
import type * as acp from "@agentclientprotocol/sdk";
import type { SessionRepositoryPort } from "#runtime/modules/session/application/ports/session-repository.port";
...
} from "./supervisor-permission.service";
...
function makePlanEntry(content: string): {
  content: string;
  status: string;
  priority: string;
} {
...
function makeService({
  ...
  return new SupervisorPermissionService({
    sessionRuntime:
      sessionRuntime ?? (vi.fn() as unknown as SessionRuntimePort),
    sessionRepo: sessionRepo ?? (vi.fn() as unknown as SessionRepositoryPort),
    decisionPort:
      decisionPort ?? (vi.fn() as unknown as SupervisorDecisionPort),
