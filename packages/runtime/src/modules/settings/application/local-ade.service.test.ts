// biome-ignore-all lint/suspicious/noTemplateCurlyInString: Tests intentionally assert literal template placeholders in Local ADE command fixtures.
// biome-ignore-all lint/suspicious/noMisplacedAssertion: Shared test helpers assert their own invariants for compact fixture setup.
// biome-ignore-all lint/suspicious/useAwait: Async test doubles keep runtime port-compatible signatures.
// biome-ignore-all lint/complexity/noVoid: Fire-and-forget test setup mirrors runtime event behavior.
...
import type { LogEntry, LogQuery } from "#runtime/shared/types/log.types";
...
import { matchesLogQuery } from "#runtime/shared/utils/log-query.util";
    ...
    getActiveId: () => {
      if (options.failProjectActiveIdRead) {
        return Promise.reject(
          new Error("project getActiveId should not be used")
        );
      }
...
test("upserts toggles and runs project hooks with redacted output", async () => {
  ...
  try {
    ...
    if (previousAllowed === undefined) {
      process.env.LOCAL_ADE_HOOK_ALLOWED = undefined;
    } else {
      process.env.LOCAL_ADE_HOOK_ALLOWED = previousAllowed;
    }
    if (previousBlocked === undefined) {
      process.env.LOCAL_ADE_HOOK_BLOCKED = undefined;
    } else {
...
test("upserts toggles and runs project plugins with redacted output", async () => {
  ...
  try {
    ...
    if (previousAllowed === undefined) {
      process.env.LOCAL_ADE_PLUGIN_ALLOWED = undefined;
    } else {
      process.env.LOCAL_ADE_PLUGIN_ALLOWED = previousAllowed;
    }
    if (previousBlocked === undefined) {
      process.env.LOCAL_ADE_PLUGIN_BLOCKED = undefined;
    } else {
