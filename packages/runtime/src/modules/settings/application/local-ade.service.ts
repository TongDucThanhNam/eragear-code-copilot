// biome-ignore-all lint/performance/useTopLevelRegex: Legacy Local ADE snapshot assembly is intentionally consolidated during the runtime extraction; normalize regex constants in a focused cleanup.
// biome-ignore-all lint/complexity/noExcessiveCognitiveComplexity: Legacy Local ADE aggregation functions are preserved to avoid behavior churn during the Electron-first migration.
// biome-ignore-all lint/style/noNestedTernary: Legacy Local ADE derived-view mapping is preserved during extraction; flattening it belongs in a focused UI-state cleanup.
// biome-ignore-all lint/complexity/noVoid: Fire-and-forget maintenance calls are preserved during migration to avoid changing runtime timing.
// biome-ignore-all lint/suspicious/noEmptyBlockStatements: Intentional no-op callbacks are preserved in legacy Local ADE helpers.
// biome-ignore-all lint/suspicious/useAwait: Async callback shapes are preserved for runtime port compatibility.
...
import type { LogEntry, LogLevel } from "#runtime/shared/types/log.types";
import type {
  ChatSession,
  SessionModelState,
...
const DASHBOARD_PARITY: LocalAdeWorkflowParity[] = [
  {
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/projects-tab.tsx",
  },
  {
    workflow: "Sessions",
    status: "available",
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/sessions-tab.tsx",
  },
  {
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/agents-tab.tsx",
  },
  {
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/logs-tab.tsx",
  },
  {
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/settings-tab.tsx",
    reason:
  ...
  },
  {
    ...
    sourceFile:
      "packages/runtime/src/presentation/dashboard/components/auth-tab.tsx",
    blockerFile: "packages/runtime/src/transport/http/routes/admin.ts",
    reason:
...
function latestAutomationRunFinishedAt(
  runs:
    | Pick<LocalAdeHookRun, "hookId" | "finishedAt">[]
    | Pick<LocalAdePluginRun, "pluginId" | "finishedAt">[],
  kind: "hook" | "plugin",
  itemId: string
): string | undefined {
...
function createRunAuditStats(
  allRuns: Pick<LocalAdeHookRun, "status" | "reviewedAt">[],
  matchingRuns: Pick<LocalAdeHookRun, "status" | "reviewedAt">[],
...
export class LocalAdeService {
  ...
  async revokePluginRegistryTrust(
    ...
    registry.trustedFingerprint = undefined;
    registry.trustedAt = undefined;
  ...
  async updatePluginPermissionGrant(
    ...
    if (input.granted) {
      ...
      plugin.grantedPermissionFingerprint = undefined;
      plugin.permissionGrantedAt = undefined;
    }
    plugin.updatedAt = now;
    await writePluginDocument(context.rootPath, document);
    return await this.snapshot(userId);
    ...
    for (const dueSchedule of dueSchedules) {
      ...
      if (!preset) {
        ...
        if (schedule) {
          ...
          schedule.lastRunBatchId = undefined;
  ...
  async configureMcpRemoteControls(
    ...
    if (remoteControls) {
      ...
      server.remoteControls = undefined;
    }
    server.updatedAt = new Date().toISOString();
    await writeMcpDocument(context.rootPath, document);
    return await this.snapshot(userId);
