// biome-ignore-all lint/style/noNonNullAssertion: Tests intentionally assert known fixture entries after setup.
...
describe("capModelList", () => {
  ...
  describe("config options truncation", () => {
    ...
    it("currentValue in first 100 → <=100, no duplicate, reordered to front", () => {
      ...
      // No duplicate values
      const values = processed.options.map((o: { value?: string }) => o.value);
  ...
  describe("OpenCode client projection", () => {
    it("detects OpenCode agent labels", () => {
      ...
      expect(shouldStripAvailableModelsForAgent({ name: "Codex" })).toBe(false);
    });
