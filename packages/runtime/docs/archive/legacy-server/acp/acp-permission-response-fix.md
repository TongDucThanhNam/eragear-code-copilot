...
```typescript
// platform/acp/permission.ts - fallback khi session không tồn tại
...
### 1. `apps/server/src/platform/acp/permission.ts`
...
### 2. `packages/runtime/src/transport/trpc/routers/tool.ts`
...
### 3. `packages/runtime/src/modules/tooling/application/respond-permission.service.ts`
