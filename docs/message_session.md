...
- **User Messages**: Saved immediately when `sendMessage` is called (`apps/server/src/trpc/procedures/ai.ts`)
...
### Session Buffering (`apps/server/src/acp/protocol/update.ts`)
...
### Key Files
- `apps/server/src/session/storage.ts` - Message storage (appendMessage, getSessionMessages)
- `apps/server/src/acp/protocol/update.ts` - SessionBuffering, handleSessionUpdate
- `apps/server/src/trpc/procedures/ai.ts` - sendMessage with buffer flush
